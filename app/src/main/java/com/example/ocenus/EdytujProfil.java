...
public class EdytujProfil extends AppCompatActivity {
    ...
    private void showPasswordChangeDailog() {
        View view = LayoutInflater.from(this).inflate(R.layout.activity_edycja_hasla, null);
        ...
        editpass.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(oldp)) {
                Toast.makeText(EdytujProfil.this, "Current Password cant be empty", Toast.LENGTH_LONG).show();
                return;
            }
            if (TextUtils.isEmpty(newp)) {
                Toast.makeText(EdytujProfil.this, "New Password cant be empty", Toast.LENGTH_LONG).show();
                return;
            }
            dialog.dismiss();
        ...
        user.reauthenticate(authCredential)
                .addOnSuccessListener(aVoid -> user.updatePassword(newp)
                        .addOnSuccessListener(aVoid1 -> {
                            pd.dismiss();
                            Toast.makeText(EdytujProfil.this, "Changed Password", Toast.LENGTH_LONG).show();
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                ...
                if (uriTask.isSuccessful()) {
                    ...
                    databaseReference.child(firebaseUser.getUid()).updateChildren(hashMap).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        public void onFailure(@NonNull Exception e) {
                            pd.dismiss();
                            Toast.makeText(EdytujProfil.this, "Error Updating ", Toast.LENGTH_LONG).show();
                        }
                    });
                } else {
                    pd.dismiss();
                    Toast.makeText(EdytujProfil.this, "Error", Toast.LENGTH_LONG).show();
                }
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
